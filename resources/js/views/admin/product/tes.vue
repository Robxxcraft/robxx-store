...
<template>
  <!-- <div>
    <button class="btn btn-primary"><router-link :to="{name: 'CreateProduct'}" style="color: white;">+</router-link></button>
      <table class="table table-bordered">
        ...
        <tbody>
          <tr v-for="product in getProducts" :key="product.id">
            ...
            <td>
              <ul>
                <li v-for="(tag, index) in product.tag" :key="index">{{tag.name}}</li>
              </ul>
            </td>
